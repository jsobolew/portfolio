...
//Init LCD
	...
	BSP_TS_InitEx(256, 256, TS_ORIENTATION_LANDSCAPE);
	...
	if (WIFI_TryConnecting() == WIFI_STATUS_OK)
	{
		//Where user touched the screen
		uint8_t TouchPosition = 120;
		...
		while (1)
		{
			TouchPosition = WIFI_ReceiveInitialValue();
			if (TouchPosition > 0)
			{
				HAL_TIM_Base_Start_IT(&htim6);
//				Display_Update(TouchPosition);
				while (1)
				{
					//Get x coord of 1st touch or 0
					TouchPosition = Display_GetTouchPosition();

					if (TouchPosition > 0)
					{
						//Light up screen if needed
						Display_ToggleScreenBacklight(GPIO_PIN_SET);

						//if user touched screen clear sleep timer
						...
						if (BSP_PB_GetState(BUTTON_WAKEUP) != GPIO_PIN_RESET)
						{
							...
							while (BSP_PB_GetState(BUTTON_WAKEUP) != GPIO_PIN_RESET)
								;
							QSPI_WRITE(TouchPosition);
						}
						Display_Update(TouchPosition);
						if (WIFI_SendValue(TouchPosition) != WIFI_STATUS_OK)
						{
							Display_ShowErrorMessage((uint8_t*) "ERROR : Failed to send Data.");
							break;
						}
					}
					...
					if (BSP_PB_GetState(BUTTON_WAKEUP) != GPIO_PIN_RESET)
					{
						//If user pressed button, clear sleep update
						__HAL_TIM_CLEAR_IT(&htim6, TIM_IT_UPDATE);
						TouchPosition = QSPI_READ();
						if (TouchPosition != -1)
						{
							Display_Update(TouchPosition);
							if (WIFI_SendValue(TouchPosition) != WIFI_STATUS_OK)
...
//					HAL_TIM_Base_Stop_IT(&htim6);
			...
			{
				Display_ShowErrorMessage((uint8_t*) "ERROR : Failed to Receive Data.");
				break;
			}
		}
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim)
{
	if (htim == &htim6)
	{
		Display_ToggleScreenBacklight(GPIO_PIN_RESET);
	}
}
...
static void MX_TIM6_Init(void)
{
	...
	//will generate an interrupt after 10s
	htim6.Instance = TIM6;
